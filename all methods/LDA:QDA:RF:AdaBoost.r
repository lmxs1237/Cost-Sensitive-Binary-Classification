...
#LDA
library(MASS)
lda.fit  = lda(Class~CT+CellSize+CellShape+MA+SE_CellSize+BN+BC+NN+Mitoses, data = data.train)
#lda.fit
...
#QDA
qda.fit = qda(Class~CT+CellSize+CellShape+MA+SE_CellSize+BN+BC+NN+Mitoses, data = data.train)
qda.fit
...
data.test = data[-train,]
data.train = data[train,]
bag.fit=randomForest(Class~CT+CellSize+CellShape+MA+SE_CellSize+BN+BC+NN+Mitoses,data=data.train,mtry=3)
...
adaboost<-boosting(Class~CT+CellSize+CellShape+MA+SE_CellSize+BN+BC+NN+Mitoses, data=data.train, boos=TRUE, mfinal=20,coeflearn='Breiman')
#for(i in 1:20){
